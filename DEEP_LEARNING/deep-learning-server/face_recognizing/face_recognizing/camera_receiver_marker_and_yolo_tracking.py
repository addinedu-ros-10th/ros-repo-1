...
# 큰 수신 버퍼 확보 (커널이 허용하는 한도 내에서)
    ...
    if SHOW_WINDOW:
        cv2.namedWindow("UDP MJPEG", cv2.WINDOW_AUTOSIZE)
    ...
    try:
        ...
        # 완성?
            ...
            if key_input == ord('q') or process_stop_flag == True:
                process_stop_flag = True
                break
...
def yolo_tracking():
    ...
    # 로봇 제어 클래스 인스턴스 생성
    dock_ctrl = PinkyDockController_YOLO(node)
        ...
        while True:
            ...
            if len(entry["chunks"]) == entry["total"]:
                ...
                if key_input == ord('q') or process_stop_flag is True:
                    ...
                    node.destroy_node()
                    break
                elif key_input == ord('p') or process_basic_flag is True:
                    process_basic_flag = False
                    yolo_switch = False
                    marker_switch = False
                    sock.close()
...
def marker_tracking():
    ...
    dock_ctrl = PinkyDockController_Marker(
        node,
...
def main():
    ...
    while True:
        ...
        if yolo_switch is True and marker_switch is False:
            print("yolo")
            yolo_tracking()
        elif yolo_switch is False and marker_switch is True:
            print("marker")
            marker_tracking()
        else:
            print("basic camera")
